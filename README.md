...
- `GET /api/v1/events/recent`: 쿠폰 이름이 포함된 최근 발급 회차 5건 조회 (`status=OPEN` 필터 지원)
...
관제 대상은 최근 `OPEN` 발급 회차 5건 중에서 선택합니다. 최근 회차는 30초 간격으로 갱신되어 예약 회차가 `OPEN`으로 전환되면 목록에 표시됩니다. 관제를 중지하면 폴링 요청과 반복 타이머가 함께 취소되고 발급 회차 선택란이 다시 활성화됩니다.
